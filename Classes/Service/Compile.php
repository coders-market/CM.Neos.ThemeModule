				...
				if($fileKey === $variant) {
					if (is_array($file)){
						...
						foreach ($file as $source) {
							...
							$fontface .= "url(" . $this->resourceManager->getPublicPackageResourceUriByPath($source) . ") format('". pathinfo($this->resourceManager->getPublicPackageResourceUriByPath($source),PATHINFO_EXTENSION) ."')";
							$i++;
						}
					} else {
						$fontface .= "url(" . $this->resourceManager->getPublicPackageResourceUriByPath($file) . ") format('". pathinfo($this->resourceManager->getPublicPackageResourceUriByPath($file),PATHINFO_EXTENSION) ."')";
					}
				}
